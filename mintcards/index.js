document.addEventListener("DOMContentLoaded", async () => {
    ...
    const resp = await fetch('./contract/ArtistsTradingCard.json');
    const respJson = await resp.json();

    const contractAddress = '0x82029fee3bb86150743f0a7df46635bc7e1028d3';
    ...
    for (const token of tokens) {
        ...
        const aOwner = $('<a>', { href: 'https://rinkeby.etherscan.io/address/' + token.owner, text: token.owner });
    ...
    contract.Transfer({}, { fromBlock: 0, toBlock: 'latest' }).get((err, logs) => {
        ...
        for (const key of keys) {
            ...
            const aFrom = $('<a>', { href: 'https://rinkeby.etherscan.io/address/' + log.args._from, text: log.args._from });
            const tdFrom = $('<td>', { class: 'text-truncate' });
            aFrom.appendTo(tdFrom);
            const aTo = $('<a>', { href: 'https://rinkeby.etherscan.io/address/' + log.args._to, text: log.args._to });
            const tdTo = $('<td>', { class: 'text-truncate' });
            aTo.appendTo(tdTo);
            const aTransaction = $('<a>', { href: 'https://rinkeby.etherscan.io/tx/' + log.transactionHash, text: log.transactionHash, class: 'text-truncate' });
